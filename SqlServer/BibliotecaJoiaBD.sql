...
INSERT INTO LIVROS(ID, NOME, AUTOR, EDITORA, IDSTATUSLIVRO) VALUES (CONVERT(BINARY(36), '1b570dec-9817-4cae-a861-8fcbf296bde8'), 'Redes- guia pratico', 'Carlos Marimoto', 'Joia editora', 1);
INSERT INTO LIVROS(ID, NOME, AUTOR, EDITORA, IDSTATUSLIVRO) VALUES (CONVERT(BINARY(36), '2b580dKc-9815-4cae-a861-8fcbf296bde9'), 'Introducao C#', 'Jamaico Silva', 'Devs editora', 1);
...
JOIN STATUSLIVRO e2 ON e2.ID = e1.IDSTATUSLIVRO;
...
JOIN STATUSCLIENTE e2 ON e2.ID = e1.IDSTATUSCLIENTE;
...
ALTER TABLE USUARIO ADD IdTemp NVARCHAR(450);

-- Copiar dados da coluna ID para IdTemp, convertendo para string
UPDATE USUARIO SET IdTemp = CAST(ID AS NVARCHAR(450));
...
WHERE parent_object_id = OBJECT_ID('Usuario') AND type = 'PK';

ALTER TABLE USUARIO DROP CONSTRAINT PK__USUARIO__3214EC2753072809;

ALTER TABLE EMPRESTIMOLIVRO DROP CONSTRAINT FK__EMPRESTIM__IDUSU__33008CF0;

ALTER TABLE Usuario DROP COLUMN LOGIN, SENHA;

select * from AspNetUsers;


  SELECT TOP(1) [a].[Id], [a].[AccessFailedCount], [a].[ConcurrencyStamp], [a].[Email], [a].[EmailConfirmed], [a].[LockoutEnabled], [a].[LockoutEnd], [a].[Login], [a].[NormalizedEmail], [a].[NormalizedUserName], [a].[PasswordHash], [a].[PhoneNumber], [a].[PhoneNumberConfirmed], [a].[SecurityStamp], [a].[Senha], [a].[TwoFactorEnabled], [a].[UserName], [a].[UserType]
  FROM [AspNetUsers] AS [a]
  WHERE [a].[NormalizedUserName] = 'YCAROBATALHA@EMAIL';




-- Renomear IdTemp para Id
EXEC sp_rename 'Usuario.IdTemp', 'Id', 'COLUMN';
...
SET 
    UserName = LOGIN,
    NormalizedUserName = UPPER(LOGIN),
    PasswordHash = SENHA;
...
ALTER TABLE Usuarios DROP COLUMN LOGIN, SENHA;
...
SELECT * FROM LIVROS l WHERE l.ID = '62303561-3032-3364-2D64-3836322D3462';